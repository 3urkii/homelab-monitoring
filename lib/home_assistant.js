...
class HAClient extends EventEmitter {
  ...
  async getConversationAgentInfo(agent_id) {
    ...
    let entries = [];
    try {
      entries = await this.#sendCommand('config_entries/get');
    } catch {
      return { agent_id, domain: null, model: null, name: entity.original_name || null };
    }
    ...
    let subOpts = {};
    let subTitle = null;
    const subId = entity.config_subentry_id;
    if (subId) {
      let sub = null;
      const subs = entry.subentries;
      if (Array.isArray(subs)) {
        sub = subs.find((s) => s && s.subentry_id === subId);
      } else if (subs && typeof subs === 'object') {
        sub = subs[subId] || null;
      }
      if (!sub) {
        try {
          const list = await this.#sendCommand('config_entries/subentries/list', {
            entry_id: entry.entry_id,
          });
          if (Array.isArray(list)) sub = list.find((s) => s && s.subentry_id === subId);
        } catch {}
      }
      if (sub) {
        subOpts = { ...(sub.data || {}), ...(sub.options || {}) };
        subTitle = sub.title || null;
      }
    }
    const opts = { ...(entry.data || {}), ...(entry.options || {}), ...subOpts };
    const model =
      opts.chat_model ||
      opts.model ||
      opts.llm_model ||
      opts.completion_model ||
      null;
    return {
      agent_id,
      domain: entry.domain || null,
      model: typeof model === 'string' && model ? model : null,
      name: subTitle || entry.title || entity.original_name || null,
    };
  }
