...
- Write plain text only. Do not use emojis or emoticons (the reply is read aloud, and emojis are spoken as their names).
- The "translation" field is ONLY for on-demand vocabulary help: a short English gloss of the specific word or phrase the learner asked about. Never translate your own reply. During normal conversation, leave it as an empty string.

Respond with ONLY a JSON object, no other text, of this exact shape:
{"reply": "<your Spanish conversational turn - the ONLY text that will be read aloud>", "correction": "<brief note (English ok) on errors in the learner's LAST message, or empty string if none>", "translation": "<empty string for normal conversation; only a short English gloss of a specific word/phrase the learner explicitly asked about — NEVER a translation of your reply>"}`;
...
function validateMessages(messages) {
  ...
  for (const m of trimmed) {
    if (!m || typeof m !== 'object'
      ...
      return { ok: false, error: 'each message needs role user|assistant and content 1..2000 chars' };
    }
  }
